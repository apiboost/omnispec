...
git tags and on npm, which semantic-release manages. The registry of record is the public npmjs.org.
...
| Commit | Bump |
|--------|------|
| `fix:` / `hotfix:` | patch (e.g. 1.0.0 to 1.0.1) |
| `feat:` | minor (e.g. 1.0.0 to 1.1.0) |
| `feat!:` or a `BREAKING CHANGE:` footer | major (e.g. 1.0.0 to 2.0.0) |
...
`1.1.0-dev.1` under the **`dev`** dist-tag. Install it with:

```bash
npm i @apiboost/omnispec@dev        # latest dev build
npm i @apiboost/omnispec@1.1.0-dev.1 # a specific one
```
